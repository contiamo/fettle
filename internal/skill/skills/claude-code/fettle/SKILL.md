...
**Scan the repo for generated code before locking the exclude list.** The language table below is a starting point, not a complete inventory — generated-file patterns are framework- and repo-specific. Look for:

- **Generator config files** at the repo root or known locations: `buf.yaml` / `buf.gen.yaml` (protobuf), `gqlgen.yml` (GraphQL), `openapi.yaml` / `openapi-generator-config.yaml`, `sqlc.yaml`, `prisma/schema.prisma`, `.proto` files, etc. These tell you what's being generated.
- **"DO NOT EDIT" / "Code generated by" markers** in candidate files. Quick verification: `grep -l "DO NOT EDIT" <a-few-sample-files>` confirms in seconds.
- **Naming conventions** for the framework in use: `*_pb.go` / `*_grpc.pb.go` (protobuf-go), `*_pb2.py` / `*_pb2_grpc.py` (protobuf-python), `*.gen.ts`, `*.openapi.{go,ts}`, `*_templ.go` (templ), `*_generated.dart`, etc.
- **`//go:generate` directives**: `grep -rn "go:generate" <repo>` reveals what's generated locally by `go generate`.

Surface what you find to the user before adding to `--exclude` — they may want some categories scanned anyway (e.g. for an OpenAPI-spec audit). Generated files are usually long and repetitive, and any findings against them are typically only actionable by changing the generator config, so they waste scan budget.
